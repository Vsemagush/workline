...
function useGroupedItems(items) {
   return useMemo(() => {
      ...
      for (let item of items) {
         if (item.theme in data) data[item.theme].push(item);
         else data[item.theme] = [item];
      }
...
function LearningPage() {
   ...
   const lineProgress = useCurrentLineProgress(
      useMatchingData(items, progress),
   );

   /** Смена текущего задания для выполнения */
   const changeProcessingItem = useCallback(() => {
      if (!items || !groupedItems || !groupedItems.length) return;
      let newItems = items.slice();
      let current = newItems.findIndex(
         (element) => element.state === STATE_PROCESSING,
      );
      ...
      let cGroup = groupedItems.findIndex(
         (group) => group.theme === newItems[current].theme,
      );
      ...
      if (currentInGroup + 1 < group.items.length) {
         ...
         if (
            cGroup + 1 < groupedItems.length &&
            groupedItems[cGroup + 1].items
         ) {
   ...
   useEffect(() => {
      ...
      db.current.subscribeChanges(
         'progress/' + db.current.getUser(),
         (result) => {
            setProgress(result);
         },
      );
   ...
   useEffect(() => {
      if (items && items.length) {
         ...
         const currentProcess = items.find(
            (element) => element.state === STATE_PROCESSING,
         );
         if (currentProcess) {
         ...
         <Pane>
            ...
            >
               <div className="progress">
                  <ProgressBar progress={lineProgress} />
               </div>
               <OrderedList>
                  {groupedItems.map((group) => {
                     return (
                        <Fragment key={group.id}>
                           <hr />
                           <Task
                              state={group.state || STATE_CLOSED}
